...
# parameters.txt contains the parameters used on the supported repos
# automatic111, invoke-ai, sd-webui
# In the order they will appear on the info window.
# Add missing parameters there when asked.
...
def click_grid_image(image_data, idx):
    ...
    image_folder = os.path.dirname(file)
...
def generate_image_list(search_string, parameter_string, first=False, master=False):
    ...
    if master:
        ...
        style = ttk.Style()
        style.configure('TProgressbar', relief='flat', borderwidth=0, background=FONT_COLOR, foreground=ALERT_COLOR)
    ...
    if new_pars:
        tk.messagebox.showinfo(title='New parameter found.',
                               message=f'Found {len(new_pars)} new parameter(s).\n'
                               f'{new_pars}\n'
                               'Add them to parameters.txt in the order you want '
                               'them to appear and run Diffusion Browser again.',
                               parent=root)
...
def main():
    ...
    butt_path = tk.Button(brd_bt_refr, text='!', bd=0, command=refresh_grid,
                          bg=FONT_COLOR, fg=BG_COLOR, activebackground=ACC_COLOR1)
    ...
    # Frame for the info
    info_frame = tk.Frame(frame_main, bg=BG_COLOR, width=INFO_IMG_SZ, height=GRID_IMG_SZ * ROW_NBR)
    ...
    text_info_height = GRID_IMG_SZ * ROW_NBR - INFO_IMG_SZ
    ...
    search_frame = tk.Frame(bottom_frame, bg=ALERT_COLOR)
    ...
    brd_bt_folder.place(x=folder_butt_x, height=BUTT_HEIGHT, width=INFO_IMG_SZ)
